...
public class AuctionCollector implements ChannelCollector {
    ...
    public List<RawOrderDto> collect(Channel channel) {
        try {
            // Mock up 파일 읽기
            ClassPathResource resource = new ClassPathResource("mock/ebay/auction_orders.json");
            AuctionOrderResponse response = objectMapper.readValue(resource.getInputStream(), AuctionOrderResponse.class);

            AuctionOrderData data = response.getData();
    ...
    private RawOrderDto toRawOrderDto(AuctionOrder order) {
    ...
    private RawOrderItemDto toRawOrderItemDto(AuctionOrder order) {
